// ===== CONFIGURATION =====
    const GOOGLE_SHEET_URL = "https://opensheet.elk.sh/1ffxYaGlkGJ5ueBURWAvIFXZszX6BTpv1Rk4WVqowA9E/Sheet1"; 
    const CONTACT_METHOD = ""; 
    const CONTACT_NUMBER = ""; 
    const MESSENGER_ID = "189877567543733"; 
    ...
    function displayProducts(data) {
      ...
      data.forEach(prod => {
        ...
        card.className = "card";
        card.innerHTML = `
          <img src="${prod.Image}" alt="${prod.Nom} onclick="openLightbox('${prod.Image}')">
          <div class="card-content">
            <h3>${prod.Nom}</h3>
            <p>${prod.Description || ""}</p>
            <p><b>${prod.Prix} Ar</b></p>
            <button onclick="addToCart('${prod.Nom}', '${prod.Prix}')">Ajouter au panier</button>
          </div>
        `;
